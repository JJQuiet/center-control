...
const sendRequest = async (output, input) => {
  ...
  if (activeTab.value === "V") {
    ...
    if(data.value[activeTab.value][rowNum][colNum]) {
      instruction = output +"$A" + input + "."
    }else {
      instruction = input +"A" + output + "."
    }
  };
  //参数instruction为指令
  console.log('%c [ instruction ]-139', 'font-size:13px; background:pink; color:#bf2c9f;', instruction)
  fetch(`http://192.168.0.176:8081/change_status?instruction=${instruction}`).then((res) => {
    res.text().then((text) => {
      if (text === "ChangeSuccess") {
        fetch(`http://192.168.0.176:8081/status`);
      }
    });
  });
};
...
const connectWebSocket = async () => {
  webSocket = new WebSocket("ws://192.168.0.176:8081/ws");
  webSocket.onopen = () => {
    // 处理连接建立
  ...
  webSocket.onmessage = (event) => {
    ...
    for (let i = 0; i < event.data.length; i += 10) {
      ...
      let input, output;
      if (event.data.substring(i + 2, i + 5) === "OFF") continue;
      if (type === "B" || type === "V") {
...
onMounted(async () => {
  ...
  await fetch(`http://192.168.0.176:8081/status`);
});
</script>
<style>
.header-col,
